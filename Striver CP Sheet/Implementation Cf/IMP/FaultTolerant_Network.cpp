...
void solve(){
    ...
    cin >> a >> b;
    ll ans = 1e18;
    vector<ll> c1 = {0,help(b,a[0]),n-1};
    vector<ll> c2 = {0,help(b,a[n-1]),n-1};

    for(auto i:c1){
        for(auto j:c2){
            ll cost = abs(a[0]-b[i]) + abs(a[n-1]-b[j]);
            if(i>0 and j>0)
                cost += abs(b[0]-a[help(a,b[0])]);
            if(i<n-1 and j<n-1)
                cost += abs(b[n-1]-a[help(a,b[n-1])]);
            ans = min(ans,cost);
        }
    }
